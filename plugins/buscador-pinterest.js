...
async function sendAlbumMessage(jid, medias, options = {}) {
  ...
  for (const media of medias) {
    if (!media.type || (media.type !== "image" && media.type !== "video")) {
      throw new TypeError(
        `media.type must be "image" or "video", received: ${media.type} (${media.type?.constructor?.name})`
      );
    }
    if (!media.data || (!media.data.url && !Buffer.isBuffer(media.data))) {
      throw new TypeError(
        `media.data must be object with url or buffer, received: ${media.data} (${media.data?.constructor?.name})`
      );
    }
  }
...
let handler = async (m, { conn, text }) => {
  if (!text) return m.reply(`🍫 Insira um texto. Exemplo: .pinterest Crow`);
  ...
  try {
    ...
    if (!results || results.length === 0)
      return conn.reply(
        m.chat,
        `Não foram encontrados resultados para "${text}".`,
        m
      );
    ...
    await sendAlbumMessage(m.chat, medias, {
      caption: `◜ Pesquisa no Pinterest ◞\n\n≡ 🔎 Busca: "${text}"\n≡ 📄 Resultados: ${maxImages}`,
      quoted: m,
    });

    await conn.sendMessage(m.chat, { react: { text: "✅", key: m.key } });
  } catch (error) {
    conn.reply(m.chat, "Erro ao obter imagens do Pinterest", m);
  }
};
