...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  try {
    if (!text)
      throw conn.reply(
        m.chat,
        `🚩 *Formato incorreto*\nExemplo:\n\n${usedPrefix + command} TV Tela plana`,
        m,
        rcanal
      );
    let res = await mercado(text);
    let libre = "`🚩 M E R C A D O - L I V R E 🚩`\n\n";
    const limit = 15;
    for (let i = 0; i < limit && i < res.length; i++) {
      let link =
        res[i].link.length > 30
          ? res[i].link.substring(0, 30) + "..."
          : res[i].link;
      libre += `*• Nome:* ${res[i].title}\n*• Estado:* ${res[i].state}\n*• Link:* ${res[i].link}\n`;
      libre += "\n" + "••••••••••••••••••••••••" + "\n";
    }
...
async function mercado(query) {
  try {
    ...
    const results = $(".ui-search-layout__item")
      .map((i, element) => {
        const title = $(element).find(".ui-search-item__title").text();
        const state = $(element)
          .find(".ui-search-item__group__element")
          .last()
          .text()
          .trim();
