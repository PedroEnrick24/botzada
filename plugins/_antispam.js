...
handler.before = async function (
  m,
  { conn, isAdmin, isBotAdmin, isOwner, isROwner, isPrems }
) {
  ...
  const messageLimit = 10; // quantidade de mensagens nesse tempo

  let time, time2, time3, mensagem, motivo;
  time = 30000; // 30 segundos
  time2 = 60000; // 1 minuto
  time3 = 120000; // 2 minutos
  ...
  if (!(sender in userSpamData)) {
    ...
    if (userData.antiBan === 1) {
      if (userData.message < 1) {
        userData.message++;
        motivo = `Por favor, não faça spam.`;
      ...
      if (userData.message2 < 1) {
        userData.message2++;
        motivo = `Você está fazendo spam novamente. Isso pode resultar em remoção.`;
        await conn.reply(m.chat, motivo, m, { mentions: [m.sender] });
        user.messageSpam = motivo;
      }
      ...
      if (userData.message3 < 1) {
        userData.message3++;
        motivo = `Você será removido(a) por fazer spam.`;
        await conn.reply(m.chat, motivo, m, { mentions: [m.sender] });
        user.messageSpam = motivo;
    ...
    if (timeDifference <= timeWindow) {
      ...
      if (userData.messageCount >= messageLimit) {
        ...
        const warningMessage = `⚠️ Detecção de spam ativada.\n\nUsuário: ${mention}`;
