    ...
    return new Promise((resolve) =>
      setInterval(async function () {
        if (!global.chatgpt.READ) {
          clearInterval(this);
          resolve(
            global.chatgpt.data === null
              ? global.loadChatgptDB()
              : global.chatgpt.data
          );
        }
      }, 1 * 1000)
...
│ ${chalk.blueBright("╭┄┈┅┈┄┈┅┈┄┅┈┄┈┅┄┈┅┈┄")}
│ ${chalk.blueBright("┊")} ${chalk.blue.bgBlue.bold.cyan(
      "MÉTODO DE VINCULACIÓN"
    )}
│ ${chalk.blueBright("╰┄┈┅┈┄┈┅┈┄┅┈┄┈┅┄┈┅┈┄")}   
│ ${chalk.blueBright("╭┄┈┅┈┄┈┅┈┄┅┈┄┈┅┄┈┅┈┄")}     
│ ${chalk.blueBright("┊")} ${chalk.bold.redBright(
      `⇢  Opción 1:`
    )} ${chalk.greenBright("Código QR")}
│ ${chalk.blueBright("┊")} ${chalk.bold.redBright(
      `⇢  Opción 2:`
    )} ${chalk.greenBright("Codígo de 8 digitos")}
│ ${chalk.blueBright("╰┄┈┅┈┄┈┅┈┄┅┈┄┈┅┄┈┅┈┄")}
│ ${chalk.blueBright("╭┄┈┅┈┄┈┅┈┄┅┈┄┈┅┄┈┅┈┄")}     
    ...
    if (!/^[1-2]$/.test(opcion)) {
      console.log(
        chalk.bold.redBright(
          `NO SE PERMITE NÚMEROS QUE NO SEAN ${chalk.bold.greenBright(
            "1"
          )} O ${chalk.bold.greenBright(
            "2"
          )}, TAMPOCO LETRAS O SÍMBOLOS ESPECIALES.\n${chalk.bold.yellowBright(
            "CONSEJO: COPIE EL NÚMERO DE LA OPCIÓN Y PÉGUELO EN LA CONSOLA."
          )}`
        )
      );
    }
...
if (opts["server"])
  (await import("./lib/server.js")).default(global.conn, PORT);
...
async function connectSubBots() {
  ...
  console.log(
    chalk.bold.greenBright(`🍭 Todos los Sub-Bots se conectaron con éxito.`)
  );
}
...
(async () => {
  ...
  const mainBotAuthFile = "CrowSession";
  try {
    ...
    console.error(
      chalk.bold.cyanBright(`🥀 Error al iniciar CrowBot-ST: `, error)
    );
  }
})();
...
global.reload = async (_ev, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (filename in global.plugins) {
      if (existsSync(dir))
        conn.logger.info(` SE ACTULIZADO - '${filename}' CON ÉXITO`);
      else {
    ...
    if (err)
      conn.logger.error(
        `SE DETECTO UN ERROR DE SINTAXIS | SYNTAX ERROR WHILE LOADING '${filename}'\n${format(
          err
        )}`
      );
    else {
      try {
        ...
        conn.logger.error(
          `HAY UN ERROR REQUIERE EL PLUGINS '${filename}\n${format(e)}'`
        );
      } finally {
  ...
  try {
    ...
    listaDirectorios.forEach((directorio) => {
      if (statSync(`./${authFileJB}/${directorio}`).isDirectory()) {
        const DSBPreKeys = readdirSync(`./${authFileJB}/${directorio}`).filter(
          (fileInDir) => {
